...
- Вынес роль db в отдельный репозиторий, удалил роль из репозитория infra и подключил роль db через requirements.yml.
- Подключил TravisCI для репозитория db для прохождения тестов molecule в gce, добавил в README.md роли db бейдж со статусом билда.
- Настроил оповещения о билде в чат Slack.
